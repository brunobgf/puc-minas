...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        TextFileWrite writeFile = new TextFileWrite("754901_insercao.txt");

        writeFile.write("Matricula: 754901 \t" + "Tempo de execução em milisegundos: " + executionTime + "\t Numero de trocas entre elementos: " + insertSortObject.countSwitch + "\t Numero de movimentacoes no vetor: " + insertSortObject.countCompare);
...
class Insert {
    ...
    private boolean compare(Series serie1, Series serie2) {
        if (serie1.getTvChannel().compareTo(serie2.getTvChannel()) > 0) {
            countCompare++;
            return true;
        }
        else if (serie1.getTvChannel().compareTo(serie2.getTvChannel()) == 0) {
    ...
    public Series[] insertSort(Series[] sortSeriesArray, int n) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            while ((j >= 0) && (compare(sortSeriesArray[j], tmp) == true)) {
